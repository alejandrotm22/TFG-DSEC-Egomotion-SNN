...
def est_self_motion_pseudo_inverse(X, Y, Dx, Dy, f, InvZ):
    ...
    output = np.concatenate((Vel, Omega))
...
if __name__ == '__main__':
    ...
    for archivo in archivos:
        print(f"Calculando el ego-motion para el archivo: {archivo}")
        ...
        for i, file in tqdm(enumerate(of_list), total=len(of_list)):
            # print("--------------------------------")
            ...
            # El primero no lo tiene en cuenta la otra red a la hora de generar el optical flow
            if i != 0: 
                gt_ego_motion.append(ego_motion)


            # Código para crear el optical flow a partir de los valores de ego motion
            ...
            ### Código para mostrar el gráfico del ego-motion
            # Extraer valores de Vel y Omega
            # vel_x.append(ego_motion[0])
            # vel_y.append(ego_motion[1])
            # vel_z.append(ego_motion[2])
        
            # omega_x.append(ego_motion[3])
            # omega_y.append(ego_motion[4])
            # omega_z.append(ego_motion[5])
